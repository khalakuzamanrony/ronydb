...
export const HomePage: React.FC<HomePageProps> = ({
  ...
  React.useEffect(() => {
    ...
    const fetchUrls = async () => {
      const { data: pdfData } = supabase.storage
        .from("files")
        .getPublicUrl("Khalekuzzaman_Rony-SQA.pdf");
      setResumeUrl(pdfData.publicUrl);
      const { data: imgData } = supabase.storage
        .from("files")
        .getPublicUrl("khalekuzzamanRony.png");
  ...
  const tabOrder = cvData.tabOrder || [
    "basics",
    "contacts",
    "work",
    "education",
...
`
            ...
            <div className="p-6 pt-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 gap-y-4">
                {tab.customFields.map((field: CustomField) => (
                  <div
                    key={field.id}
                    className="bg-sectionheader flex items-center justify-between p-3 bg-row rounded-lg break-inside-avoid overflow-hidden break-words"
                  >
  ...
  const renderSection = (sectionName: string) => {
    ...
    switch (sectionName) {
                        ...
                        <div className="flex flex-row items-center mt-3 gap-2">
                          ...
                          <button
                            onClick={() =>
                              downloadFile(
                                cvData.basics.image,
                                getFilenameFromUrl(cvData.basics.image)
                              )
                            }
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-2 w-full overflow-hidden">
                        <div className="flex-1 min-w-0">
                          <h1 className="text-3xl font-bold text-primary truncate min-w-0">
                            {cvData.basics.name}
                          </h1>
                        </div>
                        <CopyButton
                      ...
                      <div className="flex items-center justify-between mb-4 w-full overflow-hidden">
                        <div className="flex-1 min-w-0">
                          <p className="text-xl text-primary font-medium truncate">
                            {cvData.basics.label}
                          </p>
                        </div>
                        <CopyButton
                      ...
                      <div className="flex items-center justify-between w-full overflow-hidden">
                        <div className="flex-1 min-w-0">
                          <p className="text-secondary break-words whitespace-pre-line">
                            {cvData.basics.summary}
                          </p>
                        </div>
                        <CopyButton
                              ...
                              <button
                                onClick={() =>
                                  forceDownloadFromUrl(
                                    cacheBustedUrl(cvData.basics.resume ?? ""),
                                    getFilenameFromUrl(
                                      cvData.basics.resume ?? "resume"
                                    )
                                  )
                                }
                                className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-copybg hover:bg-accent text-primary hover:text-white transition-colors duration-200"
                        ...
                        <div className="p-4 bg-sectionheader rounded-lg flex flex-col justify-center">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center">
                              ...
                              <span className="font-medium text-secondary">
                                {cvData.basics.customLink1Label ||
                                  "Custom Link 1"}
                              </span>
                            </div>
                            <div className="flex space-x-2 items-center">
                        ...
                        <div className="p-4 bg-sectionheader rounded-lg flex flex-col justify-center">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center">
                              <FileText className="w-5 h-5 text-purple-600 mr-2" />
                              <span className="font-medium text-secondary">
                                {cvData.basics.customLink2Label ||
                                  "Custom Link 2"}
                              </span>
                            </div>
                            <div className="flex space-x-2 items-center">
                      ...
                      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                        {cvData.basics.customFields
                          .filter((field) => {
                            ...
                            return (
                              label !==
                                "resume_compressed (google drive)".toLowerCase() &&
                              label !== "resume (google drive)".toLowerCase()
                            );
                          })
                          .map((field, idx) => (
                            <div
                              key={field.id}
                              className="p-4 bg-sectionheader rounded-lg flex flex-col justify-center min-h-[88px]"
                            >
                              <CustomFieldRenderer field={field} />
                            </div>
                          ))}
      ...
      case "contacts":
        ...
        const getSocialIcon = (network: string) => {
          switch ((network || "").toLowerCase()) {
            ...
            case "messenger":
              return (
                <FaFacebookMessenger className="w-5 h-5 text-blue-500 mr-3" />
              );
            case "instagram":
            ...
            case "snapchat":
              return (
                <FaSnapchatGhost className="w-5 h-5 text-yellow-400 mr-3" />
              );
            case "pinterest":
            ...
            case "stack overflow":
              return (
                <FaStackOverflow className="w-5 h-5 text-orange-400 mr-3" />
              );
            default:
        ...
        const basicContacts = [
          ...
          },
          {
            ...
            label: `${cvData.contacts.location.city}, ${cvData.contacts.location.country}`,
            type: "location",
            value: `${cvData.contacts.location.city}, ${cvData.contacts.location.country}`,
            isLink: false,
            isProfile: false,
            network: undefined,
            username: undefined,
          ...
          {
            icon: <Globe className="w-5 h-5 text-blue-600 mr-3" />,
            label: cvData.contacts.url,
            type: "url",
                ...
                <div className="p-6 pt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div className="space-y-4 md:pr-4 md:border-r md:border-border ">
                      {basicCol1.map(
                        (item: (typeof basicContacts)[number], idx: number) => (
                          <div
                            key={idx}
                            className="flex items-center w-full justify-between p-4 bg-sectionheader rounded-lg"
                          >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div className="space-y-4 md:pr-4 md:border-r md:border-border">
                      {socialCol1.map(
                        (
                          item: (typeof socialProfiles)[number],
                          idx: number
                        ) => (
                          <div
                            key={idx}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-4 md:pr-4 md:border-r md:border-border">
                      {toolCol1.map(
                        (item: (typeof toolProfiles)[number], idx: number) => (
                          <div
                            key={idx}
                            className="flex items-center w-full justify-between p-4 bg-sectionheader rounded-lg"
                          >
                  ...
                  {cvData.contacts?.customFields &&
                    cvData.contacts.customFields.length > 0 && (
                      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        <h3 className="font-semibold text-secondary col-span-full">
                          Additional Contact Info
                        </h3>
                        {cvData.contacts.customFields.map(
                          (field: CustomField) => (
                            <div
                              key={field.id}
                              className="flex items-center justify-between p-3 bg-row rounded-lg break-inside-avoid overflow-hidden break-words"
                            >
                              <div className="flex-1">
                                <CustomFieldRenderer field={field} />
                              </div>
                  ...
                  <div className="grid grid-cols-1 gap-6">
                    ...
                    <div className="bg-sectionheader p-4 rounded-lg">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {(() => {
                          // Find skill groups by category
                          const testingMethods = cvData.skills.technical.find(
                            (group) => group.name === "Testing Methods"
                          );
                          const programmingLanguages =
                            cvData.skills.technical.find(
                              (group) => group.name === "Programming"
                            );
                          const tools = cvData.skills.technical.find(
                            (group) => group.name === "Tools"
                          );

                          return (
                            <>
                              {/* Testing Methods Card */}
                              {testingMethods && (
                                <div className="bg-card border border-border rounded-lg shadow-sm p-3">
                                  <div className="flex items-center justify-between mb-2">
                                    ...
                                    <CopyButton
                                      text={testingMethods.name}
                                      className="ml-1 flex-shrink-0"
                                    />
                                  </div>
                                  <div className="flex flex-wrap gap-2">
                                    {testingMethods.keywords.map(
                                      (skill, skillIndex) => (
                                        <div
                                          key={skillIndex}
                                          className="group flex flex-row items-center justify-between bg-sectionheader text-chiptext px-3 py-2 rounded-md text-sm font-medium w-full mb-1 "
                                        >
                                          <span className="truncate">
                                            {skill}
                                          </span>
                              ...
                              {programmingLanguages && (
                                <div className="bg-card border border-border rounded-lg shadow-sm p-3">
                                  <div className="flex items-center justify-between mb-2">
                                    ...
                                    <CopyButton
                                      text={programmingLanguages.name}
                                      className="ml-1 flex-shrink-0"
                                    />
                                  </div>
                                  <div className="flex flex-wrap gap-2">
                                    {programmingLanguages.keywords.map(
                                      (skill, skillIndex) => (
                                        <div
                                          key={skillIndex}
                                          className="group flex flex-row items-center justify-between bg-sectionheader text-chiptext px-3 py-2 rounded-md text-sm font-medium w-full mb-1 "
                              ...
                              {tools && (
                                <div className="bg-card border border-border rounded-lg shadow-sm p-3">
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {tools.keywords.map((skill, skillIndex) => (
                                      <div
                                        key={skillIndex}
                                        className="group flex flex-row items-center justify-between bg-sectionheader text-chiptext px-3 py-2 rounded-md text-sm font-medium w-full mb-1 "
                                      >
                              ...
                              {cvData.skills.methodologies.length > 0 && (
                                <div className="bg-card border border-border rounded-lg shadow-sm p-3">
                                  ...
                                  <div className="flex flex-wrap gap-2">
                                    {cvData.skills.methodologies.map(
                                      (methodology, index) => (
                                        <div
                                          key={index}
                                          className="group flex flex-row items-center justify-between bg-sectionheader text-chiptext px-3 py-2 rounded-md text-sm font-medium w-full mb-1 "
                              ...
                              {/* Other skill groups that don't match the predefined categories */}
                              {cvData.skills.technical
                                .filter(
                                  (group) =>
                                    group.name !== "Testing Methods" &&
                                ...
                                .map((skillGroup, index) => (
                                  ...
                                  >
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                      {skillGroup.keywords.map(
                                        (skill, skillIndex) => (
                                          <div
                                            key={skillIndex}
                                            className="group flex flex-row items-center justify-between bg-chipbg text-chiptext px-3 py-2 rounded-md text-sm font-medium w-full mb-1 hover:bg-accent hover:text-white transition-colors duration-200"
                                          >
                                            <span className="truncate">
                                              {skill}
                                            </span>
                    ...
                    {/* Methodologies Section moved below Programming */}
                  </div>

                  {cvData.skills?.customFields &&
                    cvData.skills.customFields.length > 0 && (
                      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {cvData.skills.customFields.map(
                          (field: CustomField) => (
                            <div
                              key={field.id}
                              className="flex items-center justify-between p-3 bg-row rounded-lg break-inside-avoid overflow-hidden break-words"
                  ...
                  <div className="space-y-4">
                    {cvData.projects.map((project, index) => (
                      ...
                      >
                        ...
                        {/* Display contributions like work highlights */}
                        {project.contributions &&
                          project.contributions.length > 0 && (
                            <div className="mt-4">
                              {/* <h4 className="text-md font-medium text-primary mb-2">Contributions:</h4> */}
                              <ul className="space-y-2">
                ...
                <div className="p-6 pt-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {cvData.languages.map((lang, index) => (
                      <div
                        key={index}
                        className="flex items-center justify-between w-full overflow-hidden p-4 bg-sectionheader rounded-lg"
                      >
                        <div className="flex-1 min-w-0">
                          <span className="font-medium text-secondary truncate">
                            {lang.language}
                          </span>
                          <span className="text-secondary ml-2 truncate">
                            ({lang.fluency})
                          </span>
                        </div>
                        <CopyButton
                  ...
                  {cvData.languages?.[0]?.customFields &&
                    cvData.languages[0].customFields.length > 0 && (
                      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {cvData.languages[0].customFields.map(
                          (field: CustomField) => (
                            <div
                              key={field.id}
                              className="flex items-center justify-between p-3 bg-row rounded-lg break-inside-avoid overflow-hidden break-words"
      ...
      case "coverLetters":
        if (!cvData.coverLetters || cvData.coverLetters.length === 0)
          return null;
        return (
          <section className="mb-8">
            <div className="bg-card border border-border rounded-lg shadow-md">
              ...
              {expanded && (
                <div className="p-6 pt-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          className="w-full px-3 py-2 border border-border rounded-lg bg-bg text-primary focus:outline-none focus:ring-2 focus:ring-blue-500 bg-sectionheader"
                          data-replace="$company_name"
                          value={coverLetterFields.companyName}
                          onChange={(e) => {
                            setCoverLetterFields((prev) => ({
                              ...prev,
                      ...
                      <div className="space-y-2">
                        ...
                        <input
                          ...
                          className="bg-sectionheader w-full px-3 py-2 border border-border rounded-lg bg-bg text-primary focus:outline-none focus:ring-2 focus:ring-blue-500"
                  ...
                  <div className="space-y-4">
                    {cvData.coverLetters.map(
                      (
                        coverLetter: {
                          id: string;
                          title: string;
                          content: string;
                          customFields?: import("../types/cv").CustomField[];
                        },
                        index: number
                      ) => (
                        <div
                        ...
                        >
                          ...
                          {coverLetter.customFields &&
                            coverLetter.customFields.length > 0 && (
                              <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                {coverLetter.customFields.map(
                                  (field: CustomField) => (
                                    <div
                                      key={field.id}
                                      className="flex items-center justify-between p-3 bg-row rounded-lg break-inside-avoid overflow-hidden break-words"
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 px-2">
          <div className="bg-white rounded-lg shadow-lg p-4 sm:p-8 w-full max-w-sm">
            ...
            <form onSubmit={handleSheetModalSubmit} className="space-y-4">
              ...
              {sheetError && (
                <div className="text-red-600 text-sm">{sheetError}</div>
              )}
